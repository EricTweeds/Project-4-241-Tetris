...
const unsigned char led_pos[8] = { 6,5,4,3,2,31,29,28};

void LED_set(int n) {
	int mask = 1 << led_pos[n];
	if (n < 5) LPC_GPIO2->FIOSET = mask;
	else LPC_GPIO1->FIOSET = mask;
}

void LED_clear(int n) {
	int mask = 1 << led_pos[n];
	if (n < 5) LPC_GPIO2->FIOCLR = mask;
	else LPC_GPIO1->FIOCLR = mask;
}
